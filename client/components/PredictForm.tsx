...
export function PredictForm() {
  ...
  function applySpecsText() {
    const text =
      (document.getElementById("specsText") as HTMLTextAreaElement)?.value ||
      "";
    ...
    if (storage)
      setValue(
        "storage_gb",
        storage[2] === "tb" ? Number(storage[1]) * 1024 : Number(storage[1]),
      );
    const cpu = s.match(/(i[3579]|ryzen\s*[3579])/);
    if (cpu)
      setValue(
        "cpu",
        cpu[0].replace(/\s+/, " ").toUpperCase().replace("RYZEN", "Ryzen"),
      );
  }
  ...
  return (
    <div className="w-full grid md:grid-cols-2 gap-8 items-start">
      <form
        ...
        className="bg-white/70 dark:bg-white/5 backdrop-blur rounded-xl p-6 shadow-xl border border-white/20"
      >
        ...
        <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <Select
              onValueChange={(v) => setValue("cpu", v)}
              defaultValue="Intel i7"
            >
          ...
          <div className="sm:col-span-2">
            ...
            <div className="flex items-center gap-4">
              <Slider
                ...
                step={0.1}
                defaultValue={[rating]}
                onValueChange={(v) => setValue("rating", v[0])}
              />
          ...
          <div className="sm:col-span-2">
            ...
            <textarea
              id="specsText"
              className="mt-1 w-full rounded-md bg-background/60 border px-3 py-2 h-24"
              placeholder="e.g., 16GB RAM, 1TB SSD, Intel i7"
              {...register("specsText")}
            />
            <div className="mt-2">
              <Button
        ...
        <div className="mt-6 flex gap-3">
          ...
          <Button
            ...
            onClick={async () => {
              ...
              alert(
                `Fetched ${j.count} laptop items from public API: ${j.source}`,
              );
            }}
          >
            Fetch Sample Data
      ...
      <div className="bg-gradient-to-b from-primary/10 to-transparent border border-white/20 rounded-xl p-6 shadow-xl">
        ...
        {!result && (
          <p className="text-muted-foreground mt-3">
            Submit details to see predicted price and model internals.
          </p>
        )}
        {result && (
            ...
            {result.anomaly.is_anomalous && (
              <div className="rounded-md border border-yellow-300/40 bg-yellow-100/40 dark:bg-yellow-900/20 p-3">
                ...
                <p className="text-sm mt-1">
                  Expected range ${"{"}result.anomaly.bounds.lower.toFixed(0)
                  {"}"}–${"{"}result.anomaly.bounds.upper.toFixed(0){"}"}
                </p>
                {result.anomaly.explanation && (
                  <p className="text-sm mt-2">{result.anomaly.explanation}</p>
                )}
              </div>
            )}
            <div>
              ...
              {result.llm_debug && (
                <div className="mt-3 text-xs text-muted-foreground">
                  ...
                  <pre className="mt-1 p-2 rounded bg-background/50 text-xs">
                    {result.llm_debug}
                  </pre>
                </div>
              )}
